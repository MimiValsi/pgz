...
// NOTE: At the moment, simple path check if it's unix socket or not.
// url: postgres://user:pass@localhost:port/database
// 1st tokenize url then check type of connection on host
// port will be appended on socket path or IP port
pub fn connect(url: []const u8, alloc: std.mem.Allocator) !*Pgconn {
    const config = try cfg.Config.createConfigAlloc(url, alloc);

    // return &.{ .stream = , .config = config };
}

// fn setStream(url: []const u8)

test "config" {
    const url = "postgres://user:pass@localhost:port/database";
...
//     const pgconn = try connect(path, 5432);
